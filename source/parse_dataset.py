...
def convert_dataset(in_annotations, prompt, out_dir=None):
    ...
    for item in annotations:
        ...
        line = ("{'messages': [{'role': 'system', 'content': 'You are a professional anomaly detection and classification tool that detects objects that could prevent an excavator from digging.'}, {'role': 'user', 'content': '<image>" + prompt + "'}, {'role': 'assistant', 'content': ") + str(anomalies) + "}], 'images': ['" + img_id + "'], 'label': " + str(anomalies) + "}"

        out += line + "\n"

    with open(Path(output_dir).parent / "ann_2.jsonl", "w") as f:
        f.write(out)
